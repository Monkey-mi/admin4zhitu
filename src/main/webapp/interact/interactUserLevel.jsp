...
<script type="text/javascript">
	var maxId=0;
	htmTableTitle = "用户等级管理";
	htmTableWidth = 870;
	htmTablePageList = [10,20,30];
	myPageSize = 10;
	myRowStyler= 0;
	loadDataURL = "./admin_interact/userlevel_QueryUserlevelList";
	deleteURI = "./admin_interact/userlevel_DeleteUserlevelByIds?userLevelIds=";
	updatePageURL = "./admin_interact/userlevel_updateUserLevelByRowJson";
	addWidth = 500, //添加信息宽度
	addHeight = 390, //添加信息高度
	addTitle = "添加用户等级"; //添加信息标题
	...
	function loadAddFormValidate() {
		...
		$("#minFansCount").formValidator({empty:false,onshow:"最小粉丝数",onfocus:"例如:10",oncorrect:"输入正确"})
		.inputValidator({min:1,onerror:"请输入最小粉丝数"});
		$("#maxFansCount").formValidator({empty:false,onshow:"最大粉丝数",onfocus:"例如:20",oncorrect:"输入正确"})
		.inputValidator({min:1,onerror:"请输入最大粉丝数"});
		...
		.inputValidator({min:1,max:10,onerror:"请输入等级描述"});
	}
	
	
	function checkData(){
		if($("#minFansCount").val()>$("#maxFansCount").val()){
			alert("最小粉丝数不能大于最大粉丝数");
			return false;
		}
		if($("#minLikedCount").val()>$("#maxLikedCount").val()){
			alert("最小喜欢数不能大于最大喜欢数");
			return false;
		}
		if($("#minCommentCount").val()>$("#maxCommentCount").val()){
			alert("最小评论数不能大于最大评论数");
			return false;
		}
		if($("#minPlayTimes").val()>$("#maxPlayTimes").val()){
			alert("最小播放数不能大于最大播放数");
			return false;
		}
	}
</script>
...
<body>
	...
	<div id="tb" style="padding:5px;height:auto" class="none">
		<div>
			...
			<a href="javascript:void(0);" onclick="javascript:htmDelete(recordIdKey);" class="easyui-linkbutton" title="删除用户等级" plain="true" iconCls="icon-cut" id="delBtn">删除</a>
   		</div>
	</div>  
	
	<!-- 添加记录 -->
	<div id="htm_add">
		<form action="./admin_interact/userlevel_AddUserlevel" id="add_form" class="none" method="post">
			<table id="htm_add_table" width="470">
			  <tbody>
				<tr>
					<td class="leftTd">最小粉丝数：</td>
					<td><input id="minFansCount" name="minFansCount" class="easyui-numberbox" onchange="validateSubmitOnce=true;"/></td>
					<td class="rightTd"><div id="minFansCount_tip" class="tipDIV"></div></td>						
				</tr>
				<tr>
					<td class="leftTd">最大粉丝数：</td>						
					<td><input id="maxFansCount" name="maxFansCount" class="easyui-numberbox" onchange="validateSubmitOnce=true;"/></td>
					<td class="rightTd"><div id="maxFansCount_tip" class="tipDIV"></div></td>
				</tr>
				<tr>
					<td class="leftTd">最小喜欢数：</td>
					<td><input id="minLikedCount" name="minLikedCount" class="easyui-numberbox" onchange="validateSubmitOnce=true;"/></td>
					<td class="rightTd"><div id="minLikedCount_tip" class="tipDIV"></div></td>
				</tr>
				<tr>
					<td class="leftTd">最大喜欢数：</td>
					<td><input id="maxlikedCount" name="maxLikedCount" class="easyui-numberbox" onchange="validateSubmitOnce=true;"/></td>
					<td class="rightTd"><div id="maxlikedCount_tip" class="tipDIV"></div></td>
				</tr>
				<tr>
					<td class="leftTd">最小评论数：</td>
					<td><input id="minCommentCount" name="minCommentCount" class="easyui-numberbox" onchange="validateSubmitOnce=true;"/></td>
					<td class="rightTd"><div id="minCommentCount_tip" class="tipDIV"></div></td>
				</tr>
				<tr>
					<td class="leftTd">最大评论数：</td>						
					<td><input id="maxCommentCount" name="maxCommentCount" class="easyui-numberbox" onchange="validateSubmitOnce=true;"/></td>
					<td class="rightTd"><div id="maxCommentCount_tip" class="tipDIV"></div></td>
				</tr>
				<tr>
					<td class="leftTd">最小播放数：</td>
					<td><input id="minPlayTimes" name="minPlayTimes" class="easyui-numberbox" onchange="validateSubmitOnce=true;"/></td>
					<td class="rightTd"><div id="minPlayTimes_tip" class="tipDIV"></div></td>
				</tr>
				<tr>
					<td class="leftTd">最大播放数：</td>
					<td><input id="maxPlayTimes" name="maxPlayTimes" class="easyui-numberbox" onchange="validateSubmitOnce=true;"/></td>
				...
				<tr>
					...
					<td><input id="levelDescription" name="levelDescription" type="text" onchange="validateSubmitOnce=true;"/></td>
