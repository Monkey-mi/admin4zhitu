...
public class ADKeywordCacheServiceImpl extends BaseCacheDaoImpl<Map<String, Integer>>implements ADKeywordCacheService {
	...
	public Map<String, Integer> getADKeywordCacheFromRedis() {
		...
		BoundValueOperations<String, Map<String, Integer>> bound = getRedisTemplate()
				.boundValueOps(Admin.ADMIN_DAEMON_ADCACHEKEY);
		if (bound.get() == null) {
	...
	@Override
	public void setADKeywordCacheToDB() {
		// 先把当前的redis数据持久化到数据库
	...
	@Override
	public void refreshADKeywordCacheToRedis() {
		List<ADKeywordDTO> list = mapper.queryADKeywordList(new ADKeywordDTO());
		Map<String, Integer> map = new HashMap<String, Integer>();
