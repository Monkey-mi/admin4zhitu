		...
		buildNumberDtos("getChannelId", dto, page, rows, jsonMap,
				new NumberDtoListAdapter<ChannelBaseCountDto>() {
					...
					public List<? extends Serializable> queryList(
						...
						List<ChannelBaseCountDto> resultList = mapper
								.queryChannelBaseCountList(dto);

						// 后台查询出的trueXX字段，为展示在客户端给用户看的数据，真实数据要减去base值，故做下列循环操作，方便运营端查看真实数据
						for (ChannelBaseCountDto cbcDto : resultList) {
							cbcDto.setTrueWorldCount(cbcDto.getTrueWorldCount()
									- cbcDto.getWorldBaseCount());
							cbcDto.setTrueChildCount(cbcDto.getTrueChildCount()
									- cbcDto.getChildBaseCount());
							cbcDto.setTrueMemberCount(cbcDto
									.getTrueMemberCount()
									- cbcDto.getMemberBaseCount());
							cbcDto.setTrueSuperbCount(cbcDto
									.getTrueSuperbCount()
									- cbcDto.getSuperbBaseCount());
						}
						return resultList;
					}
